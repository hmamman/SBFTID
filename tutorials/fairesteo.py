...
# Get the absolute path to the directory where fairesteo.py is located
base_path = os.path.dirname(os.path.abspath(__file__))
# Two levels up from fairesteo.py
sys.path.append(os.path.join(base_path, "../"))

from tutorials.algorithms.esteo import ESTEO
from utils.helpers import get_experiment_params
from tutorials.fairses import FairSES


class FairESTEO(FairSES):
    def __init__(self, config, model, sensitive_param, population_size=200, threshold=0, sigma=0.1, TM_size=200, feature_size_rate=3):
        super().__init__(config, model, sensitive_param, population_size, threshold)
        self.approach_name = "FairESTEO"
        ...
        self.setup = ESTEO(
            ...
            feature_size_rate=feature_size_rate,
            TM_size=TM_size,
        )
...
if __name__ == '__main__':
    ...
    fairesteo.run(max_allowed_time=max_allowed_time)
